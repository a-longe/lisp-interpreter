...
fn add(node: &Node) -> Value {
    let proc = match &node.expr {
        Expr::Procedure(p) => p,
        _ => panic!("trying to apply add function to a value or token")
    };
    let mut sum: f64 = 0.0;
    for n in &proc.args {
        sum += match n.execute() {
...
fn sub(node: &Node) -> Value {
    let proc = match &node.expr {
        Expr::Procedure(p) => p,
        _ => panic!("trying to apply sub function to a value or token")
    };
    let mut sum: f64 = 0.0;
...
fn mult(node: &Node) -> Value {
    let proc = match &node.expr {
        Expr::Procedure(p) => p,
        _ => panic!("trying to apply mult function to a value or token")
    };
...
fn div(node: &Node) -> Value{
    let proc = match &node.expr {
        Expr::Procedure(p) => p,
        _ => panic!("trying to apply div function to a value or token")
    };
